...
void MinecraftServerManager::load_config(const std::string& path) {
    LOG_INFO("Загрузка конфигурации из: " + path, "CONFIG");
    try {
        // Проверка существования файла
        if (!fs::exists(path)) {
            throw std::runtime_error("Файл конфигурации не найден");
        }
        ...
        // Проверка и загрузка аргументов Forge
        config_.forge_args = data["server"]["forge_args"].get<std::string>();
        if (config_.forge_args[0] == '@') {
            std::string args_path = config_.server_dir + "\\" + config_.forge_args.substr(1);
            std::replace(args_path.begin(), args_path.end(), '/', '\\');
            if (!fs::exists(args_path)) {
                throw std::runtime_error("Файл аргументов Forge не найден: " + args_path);
            }
        }

        // Валидация конфигурации
        if (!fs::exists(config_.java_path)) {
            throw std::runtime_error("Java не найдена по указанному пути");
        }
        if (!fs::exists(config_.server_dir)) {
            throw std::runtime_error("Директория сервера не существует");
        }
