/*
Компиляция:
g++ main.cpp MinecraftServerManager.cpp httpServer.cpp -o mshost -lws2_32
*/
...
const std::string Version = "0.2.7a";
    ...
    while (running) {
        ...
        if (command == "server-start") {
            ...
            std::wcout << L"\nСписок доступных команд:\n"
                       ...
                       << L"\"server-stop/stop\" : Останавливает запущенный Minecraft Server\n"
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        Logger::instance().init("server.log", true, "web.log");
        LOG_INFO("===== Майнкрафт Хост v" + Version + " =====", "MAIN");

        LOG_INFO("Инициализация серверов...", "MAIN");
        MinecraftServerManager mcserver;
        HttpServer http(mcserver, 8080, running, "tokens");
        g_mc   = &mcserver;
        g_http = &http;
