...
HttpServer::HttpServer(MinecraftServerManager& manager, 
    ...
    : manager_(manager), 
      ...
      web_root_(web_root),
      upload_tickrate_(upload_tickrate)
{ 
    load_tokens(); 
}
...
void HttpServer::run() {
    ...
    if (!fs::exists(web_root_)) {
        LOG_ERR("Директория с фронтендом не найдена: " + web_root_, "WEB");
    }
    ...
    svr.Get("/api/download-modpack", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        // 6. Функция для потоковой передачи (теперь копируемая)
        auto provider = [
            file_stream,
            buffer,
            tickrate = this->upload_tickrate_
            ...
            try {
                ...
                while (length > 0 && !file_stream->eof()) {
                    auto read_size = static_cast<std::streamsize>(
                        std::min<size_t>(length, buffer->size())
                    );
